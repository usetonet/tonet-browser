...
5. **CSS (author)** — **in progress**: **tokenizer** + **top-level rule split** + **declarations** + **simple selector cascade** (`css::author_cascade`: type / `.class` / `#id`, specificity + order); **`html` / `body` type rules** supply document-wide defaults for typography-like properties (including **`text-decoration: underline`**, **`text-align`** (`left` / `start`, `center`, `right` / `end`; not `justify` yet), **`line-height`** as `normal`, unitless number, `px` / `em` / `rem`, `%`, **`letter-spacing`** as `normal` or `px` / `em` / `rem`, and **`text-transform`** as `none` / `uppercase` / `lowercase` / `capitalize` — whitespace-separated words for `capitalize`) before per-node rules; **`margin`** shorthand plus **`margin-top` / `margin-bottom`** longhands (length units) adjust vertical spacing and **do not** inherit from `html`/`body`. The shell **fetches** sheets and maps to egui. **No** full box model, combinators, true DOM inheritance tree, or pseudo-classes yet. Next: broader selectors + layout per §5.
