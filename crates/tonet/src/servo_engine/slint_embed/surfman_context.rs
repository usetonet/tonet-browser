// Adapted from Slint `examples/servo` (MIT). Original SPDX-License-Identifier: MIT
...
impl SurfmanRenderingContext {
    ...
    fn read_framebuffer_to_image(
        ...
        let w = source_rectangle.width().max(0) as usize;
        let h = source_rectangle.height().max(0) as usize;
        let stride = w * 4;
        let orig_pixels = pixels.clone();
        for y in 0..h {
            let dst_start = y * stride;
            let src_start = (h - y - 1) * stride;
