...
/// `line-height`, `letter-spacing`, `font-weight`, `font-style`, `margin` (including `margin-left` / `margin-right` and shorthand), `text-decoration`, `text-align`, `text-transform`, `text-indent`, `opacity`, `visibility`, `display` (`none` skips the node, including when `html`/`body` defaults resolve to `none`), `white-space` (`nowrap` → no soft wrap), `word-break` (`break-all`), `overflow-wrap` / `word-wrap` (`anywhere` / `break-word`), `max-width`, `padding` / `padding-left` / `padding-right` / `padding-top` / `padding-bottom`, and `background-color` (per-node only, not from `html`/`body`) override or extend built-in page chrome.
pub fn render_nodes(
    ui: &mut Ui,
    loc: Locale,
    nodes: &[DomNode],
    ...
    let fill_bg = hint
        ...
        .map(|mut bg| {
            if let Some(o) = hint.and_then(|h| h.opacity) {
                bg = bg.gamma_multiply(o);
            }
            bg
        });

    let mut paint_insets = |ui: &mut Ui| {
        let full_w = ui.available_width().max(1.0);
        let pt = hint
